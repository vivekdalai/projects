	...
	while(1){
		...
		if(strcmp(data,"LIST") == 0){
			...
			for(int i = 0 ; i < clientCount ; i ++){
				...
				if(i != index)
					l += snprintf(output + l,maxline,"Client %d is at socket %d.\n",i + 1,Client[i].sockID);

			}

			send(clientSocket,output,maxline,0);
			continue;

		}
		if(strcmp(data,"SEND") == 0){

			read = recv(clientSocket,data,maxline,0);
			data[read] = '\0';
			...
			send(Client[id].sockID,data,maxline,0);			
